...
# plot of the loglikelihood inside and outside the care-homes
log_model1 <-data.frame(i,iout)
log_model2 <-data.frame(i,oout)

inside_loglikelihood <- 
  ggplot(log_model1,aes(i)) + 
  geom_line(aes(x = i, y = iout, col='death log-likelihood function values inside the care-homes'))+
  scale_colour_manual("",
                      values=c(
                        "death log-likelihood function values inside the care-homes" = "cornflowerblue" 
                      ))+
  scale_x_continuous("Time") +
  scale_y_continuous("Normal Log-likelihood Values") +
  ggtitle("Death Log-likelihood Inside the Care-homes")

outside_loglikelihood <- ggplot(log_model2,aes(i)) + 
  geom_line(aes(x = i, y = oout, col='death log-likelihood function values outside the care-homes'))+
  scale_colour_manual("",
                      values=c(
                        "death log-likelihood function values outside the care-homes" = "cornflowerblue" 
                      ))+
  scale_x_continuous("Time") +
  scale_y_continuous("Normal Log-likelihood Values") +
  ggtitle("Death Log-likelihood Outside the Care-homes")
